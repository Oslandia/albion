...
class Scene(QObject):
    ...
    def rendergl(self, leftv, upv, eye, height, context):
        ...
        vtx = numpy.array(vtx, dtype=numpy.float32)*self.__zScale   
    ...
    def __update(self, layer):
        ...
        if layer=='label':
            ...
            self.__cur.execute("""
                select st_collect(geom) from albion.node where graph_id='{}'
                """.format(self.graph_id))
            lines = wkb.loads(self.__cur.fetchone()[0], True)
            vtx = []
            idx = []
            ...
            self.__cur.execute("""
                select st_collect(geom) from albion.edge where graph_id='{}'
                """.format(self.graph_id))
            lines = wkb.loads(self.__cur.fetchone()[0], True)
            vtx = []
            idx = []
            ...
            self.__cur.execute("""
                select st_collect(ceil_) from albion.edge where graph_id='{}'
                """.format(self.graph_id))
            lines = wkb.loads(self.__cur.fetchone()[0], True)
            vtx = []
            idx = []
            ...
            self.__cur.execute("""
                select st_collect(wall_) from albion.edge where graph_id='{}'
                """.format(self.graph_id))
            lines = wkb.loads(self.__cur.fetchone()[0], True)
            vtx = []
            idx = []
