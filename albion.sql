...
create view albion.metadata as select id, srid, close_collar_distance, snap_distance, precision, interpolation, end_distance, end_slope, correlation_distance, correlation_slope from _albion.metadata
;
...
create view albion.section as select id,  nb_cell, anchor::geometry('LINESTRING', $SRID) from _albion.section
;

alter view albion.section alter column id set default _albion.unique_id()::varchar
;
alter view albion.section alter column nb_cell set default 0
;

create or replace view albion.colar_section as
with sorted as (
    select row_number() over() as id,  rank() over (partition by s.id order by st_distance(s.anchor, c.geom)) as rk, c.id as collar_id, c.geom, s.id as section_id, s.nb_cell
    from _albion.collar as c, _albion.section as s
)
select id, collar_id, section_id, nb_cell, geom::geometry('POINTZ', $SRID)
from sorted
where rk < nb_cell
;

create or replace view albion.cell_section as
select row_number() over() as id, c.id as cell_id, c.a, c.b, c.c, c.geom::geometry('POLYGON', $SRID), s.id as section_id
from _albion.cell as c, _albion.section as s
where (select count(1) from albion.colar_section as cs join (values (c.a), (c.b), (c.c)) as t(id) on t.id=cs.collar_id where cs.section_id = s.id) = 2
;
...
-- the mesh is convex, we take the part of the convex hull of the collars that face the line
...
$$
    begin
        return (
        with hull as (
            select st_exteriorring(st_convexhull(st_collect(geom))) as geom from albion.collar
        ),
        seg as (
        ...
        facing as (
            ...
            from seg
            where ((st_x(st_endpoint(anchor)) - st_x(st_startpoint(anchor)))*(st_x(ep)-st_x(sp))
                  +(st_y(st_endpoint(anchor)) - st_y(st_startpoint(anchor)))*(st_y(ep)-st_y(sp))) > 0
        )
        select st_linemerge(st_collect(geom)) from facing
    ); 
    end;
$$
;
...
select albion.first_section(anchor) from albion.section
;
...
create or replace view albion.sorted_cell_section as
with sorted as (
    select row_number() over() as id,  rank() over (partition by s.id order by st_distance(s.anchor, c.geom)) as rk, c.id as cell_id, c.geom, s.id as section_id, s.nb_cell
    from _albion.cell as c, _albion.section as s
)
select id, cell_id, section_id, nb_cell, geom::geometry('POLYGON', $SRID)
from sorted
where rk < nb_cell
;

create or replace function albion.sorted_mesh(triangles geometry, anchor geometry)
returns bytea
...
$$
    ...
    triangles_ = wkb.loads(triangles, True)
    anchor_ = wkb.loads(anchor, True)

    distances = numpy.array([t.centroid.distance(anchor_) for t in triangles_])
    vtx = []
    vtx_map = {}
    i = 0
    idx = numpy.empty((3*len(triangles_,)), dtype=numpy.int32)
    for tri in triangles_:
        for c in  tri.exterior.coords[:-1]:
            if c in vtx_map:
                idx[i] = vtx_map[c]
            else:
                vtx_map[c] = len(vtx)
    ...
    vtx = numpy.array(vtx, dtype=numpy.float64)
    assert(vtx.shape[1]==2)
    idx = idx.reshape(-1,3)[numpy.argsort(distances)]
    return len(vtx).to_bytes(4, sys.byteorder) + len(triangles_).to_bytes(4, sys.byteorder) \
        +vtx.tobytes()+idx.tobytes()
$$
;

create or replace function albion.sorted_mesh_limit(mesh bytea, nb_cell integer, anchor geometry)
returns varchar
language plpython3u immutable
as
$$
    ...
    nb_vtx, nb_tri = numpy.frombuffer(mesh , dtype=numpy.int32, count=2)
    vtx = numpy.frombuffer(mesh, dtype=numpy.float64, count=2*nb_vtx, offset=4*2).reshape(-1,2)
    tri = numpy.frombuffer(mesh, dtype=numpy.int32, count=3*nb_tri, offset=4*2+8*2*nb_vtx).reshape(-1,3)
    ...
    def border(tri):
        ...
        for t in tri:
            if (t[1], t[0]) in edges:
                edges.remove((t[1], t[0]))
            else:
                edges.add((t[0], t[1]))
        return edges

    edges = border(tri[:nb_cell]).difference(border(tri))
    if len(edges):
        result = linemerge([LineString([vtx[e[0]], vtx[e[1]]]) for e in edges])
        geos.lgeos.GEOSSetSRID(result._geom, geos.lgeos.GEOSGetSRID(anchro_._geom))
...
create or replace function albion.next_section(mesh bytea, nb_cell integer, section_geom geometry)
returns varchar
language plpython3u immutable
as
$$
    ...
    section_geom_ = wkb.loads(section_geom, True)

    # looks for next cell
    edge = []
    t = tri[nb_cell]
    for i in range(3):
        if Point(vtx(t[i])).intersects(section_geom) and Point(vtx(t[(i+1)%3])).intersects(section_geom):
            edge.append(i)
    assert(len(edge))


    return str(border(tri[:nb_cell]).difference(border(tri)))
$$
;


select length(albion.sorted_mesh('MULTIPOLYGON(((0 0, 1 0, 0 1, 0 0)))'::geometry, 'LINESTRING(0 -1, 1 -1)'::geometry))
;

select albion.sorted_mesh_limit(albion.sorted_mesh('MULTIPOLYGON(((0 0, 1 0, 0 1, 0 0)))'::geometry, 'LINESTRING(0 -1, 1 -1)'::geometry), 1, 'LINESTRING(0 -1, 1 -1)'::geometry)
;



--create or replace function albion.next_section(current_section geometry, section_anchor geometry)
--returns geometry
--language plpython
--as
--$$
--    begin
--        current_section = coalesce(current_section, (select ));
--        
--        select id, a, b, c, 
--        st_intersects(current_section, st_pointn(st_exteriorring(s.geom), 1)) as ta,
--        st_intersects(current_section, st_pointn(st_exteriorring(s.geom), 2)) as ta,
--        st_intersects(current_section, st_pointn(st_exteriorring(s.geom), 3)) as ta
--        from _albion.cell 
--        where st_intersects(current_section, geom)
--        and st_linelocatepoint(current_section, );
--        order by st_distance(section_anchor geometry)
--    end;
--$$
--;
--
...
--select albion.project(h.geom, c.) as geom
--from _albion.hole as h
--join albion.collar_section as c on c.hole_id=h.id
--;
...
/*
            ...
            with pt as (
                select (t.d).path as p, (t.d).geom as geom from 
                    (select st_dumppoints(
                        ...
                        )) as d from albion.grid 
                    where id=grid_id
                    and st_linelocatepoint(geom, coalesce((select st_startpoint(h.geom) from albion.hole as h join albion.node as n on n.hole_id=h.id where n.id=start_), st_startpoint(new_geom))) < st_linelocatepoint(geom, coalesce((select st_startpoint(h.geom) from albion.hole as h join albion.node as n on n.hole_id=h.id where n.id=end_), st_endpoint(new_geom)))
                ) as t
            ),
            snap as (
    ...
    $$
        begin
            ...
            if tg_op = 'INSERT' then
                insert into _albion.grid(id, geom) values(new.id, new.geom) returning id into new.id;
                return new;
    ...
    $$
        ...
        begin
            if tg_op = 'INSERT' or tg_op = 'UPDATE' then
                ...
                if parent_ is not null and tg_op = 'INSERT' and
    ...
    $$
        ...
        begin
            if tg_op = 'INSERT' or tg_op = 'UPDATE' then
                ...
                -- snap top and bottom extremities to node
                select st_setpoint(st_setpoint(topgeom, 0, 
                        coalesce((select st_3dlineinterpolatepoint(geom, st_linelocatepoint(albion.to_section(geom, albion.current_section_geom()), st_startpoint(new.top))) from _albion.node where id=new.start_), 
                            st_startpoint(topgeom))), 
                        -1, coalesce((select st_3dlineinterpolatepoint(geom, st_linelocatepoint(albion.to_section(geom, albion.current_section_geom()), st_endpoint(new.top))) from _albion.node where id=new.end_), st_endpoint(topgeom))) into topgeom;

                select st_setpoint(st_setpoint(bottomgeom, 0, 
                        coalesce((select st_3dlineinterpolatepoint(geom, st_linelocatepoint(albion.to_section(geom, albion.current_section_geom()), st_startpoint(new.bottom))) from _albion.node where id=new.start_), 
                            st_startpoint(bottomgeom))), 
                    -1, coalesce((select st_3dlineinterpolatepoint(geom, st_linelocatepoint(albion.to_section(geom, albion.current_section_geom()), st_endpoint(new.bottom))) from _albion.node where id=new.end_), 
                        st_endpoint(bottomgeom))) into bottomgeom;
    ...
    $$
        begin
            ...
            select graph_id_, e.start_, e.end_, grid_id_, e.geom  from possible_edge as e
            where 
            --e.rk <= least(e.c1, e.c2) 
            --and
            not exists (select 1 from albion.edge where (start_=e.start_ and end_=e.end_) or (start_=e.end_ and end_=e.start_));
    ...
    $$
        begin
            ...
            select distinct n.geom, n.top, n.bottom, graph_id_, grid_id_ from next as n, albion.metadata as m
            where abs(st_z(st_startpoint(geom)) - st_z(st_endpoint(geom)))/st_length(geom) < m.correlation_slope;
    ...
    $$
        begin
            with extreme as (
                ...
                least(m.end_distance, .4*st_3ddistance(st_startpoint(a.top), st_startpoint(a.bottom))/greatest(m.end_slope,1e-6)) as start_distance,
            ...
            select 
                albion.to_section(
                st_makeline(
                    st_translate(st_startpoint(e.geom), -dx*e.start_distance, -dy*e.start_distance, -dz*e.start_distance),
                    st_startpoint(e.geom)
                ), s.geom), 
                albion.to_section(
                st_makeline(
                    st_translate(st_startpoint(e.top), -dx*e.start_distance, -dy*e.start_distance, -dz*e.start_distance - m.end_slope*e.start_distance),
                    st_startpoint(e.top)
                ), s.geom), 
                albion.to_section(
                st_makeline(
            ...
            select 
                ...
                st_makeline(
                    st_endpoint(e.geom),
                    st_translate(st_endpoint(e.geom), dx*e.end_distance, dy*e.end_distance, dz*e.end_distance)
                ), s.geom), 
                albion.to_section(
                st_makeline(
                    st_endpoint(e.top),
                    st_translate(st_endpoint(e.top), dx*e.end_distance, dy*e.end_distance, dz*e.end_distance - m.end_slope*e.end_distance)
                ), s.geom), 
                albion.to_section(
                st_makeline(
            ...
            -- isolated nodes
            with isolated as (
                select st_lineinterpolatepoint(albion.to_section(n.geom, g.geom), .5) as geom, 
                st_startpoint(albion.to_section(n.geom, g.geom)) as top,
            ...
            select 
                st_makeline(
                    st_translate(e.geom, -e.end_distance, 0),
                    e.geom
                ), 
                st_makeline(
                    st_translate(e.top, -e.end_distance, -m.end_slope*e.end_distance),
                    e.top
                ), 
                st_makeline(
