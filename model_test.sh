...
psql -h localhost -p 55432 niger -c "delete from albion.edge cascade;"
...
update albion.metadata set snap_distance=.3, correlation_distance=300, correlation_slope=.5;
...
select count(albion.extend_to_interpolated('tarat_u1', id)) from albion.grid;
select count(albion.extend_to_interpolated('tarat_u1', id)) from albion.grid;
EOF
...
select count(albion.extend_to_interpolated('tarat_u2', id)) from albion.grid;
select count(albion.extend_to_interpolated('tarat_u2', id)) from albion.grid;
EOF
...
select count(albion.extend_to_interpolated('tarat_u3', id)) from albion.grid;
select count(albion.extend_to_interpolated('tarat_u3', id)) from albion.grid;
EOF
...
select count(albion.extend_to_interpolated('tarat_u4', id)) from albion.grid;
select count(albion.extend_to_interpolated('tarat_u4', id)) from albion.grid;
EOF
...
select albion.auto_graph('min_u1', 'tarat_u1');
EOF
...
select albion.auto_graph('min_u2', 'tarat_u2');
EOF
...
select albion.auto_graph('min_u3', 'tarat_u3');
EOF
...
select albion.auto_graph('min_u4', 'tarat_u4');
EOF
